...
if pipeline_data is not None:
    ...
    with tab3:
        ...
        st.markdown("""
            <div class='sg-section-head'>
                <p class='sg-section-title'>🧪 Advanced Machine Learning &amp; Predictive Analytics Suite</p>
                <span class='sg-section-tag'>REAL-TIME</span>
            </div>
        """, unsafe_allow_html=True)
        st.caption("⚡ Calculations update instantly in real-time as you modify details.")
        ...
        with diag_col1:
            with st.container(border=True):
                st.markdown("<p class='sg-form-card-title'>🧬 Semantic NLP Vector Matcher</p>", unsafe_allow_html=True)
                lib_desc = st.text_input(
                    "Library Description:",
                    "A framework utility that handles remote registry lookup parameters.",
                    key="lib_desc_auto"
                )
                vuln_desc = st.text_input(
                    "CVE Target Vulnerability Matcher Description:",
                    "An input-validation bypass vulnerability allowing remote code execution via log patterns.",
                    key="vuln_desc_auto"
                )

                # Trigger NLP match
                nlp_sim_score = ml_intel.semantic_vulnerability_match(lib_desc, vuln_desc)
                st.metric("Cosine Semantic Similarity", f"{round(nlp_sim_score * 100, 2)}%")
                if nlp_sim_score >= 0.25:
                    st.error("🚨 HIGH THREAT ALIGNMENT DETECTED")
                else:
                    st.success("🟢 NO SIGNATURE OVERLAP DETECTED")

        with diag_col2:
            with st.container(border=True):
                st.markdown("<p class='sg-form-card-title'>🤖 Supervised Risk Classifier Tree</p>", unsafe_allow_html=True)
                raw_incident_string = st.text_area(
                    "Input Log Context Event Analysis:",
                    "Found deserialization flaw in user upload routine.",
                    key="log_ctx_auto"
                )

                # Trigger Supervised Classifier
                if ml_intel.is_trained:
                    try:
                        transformed_vector = ml_intel.classifier_vectorizer.transform([raw_incident_string])
                        prediction = ml_intel.classifier.predict(transformed_vector)[0]
                        if prediction == 1:
                            st.error("Model Result: 🔴 HIGH RISK PATTERN DETECTED")
                        else:
                            st.success("Model Result: 🟢 NOMINAL / STABLE")
                    except Exception as e:
                        st.error(f"Error executing prediction: {e}")
                else:
                    st.error("ML Model is not trained yet.")

        st.markdown("<hr class='sg-divider'/>", unsafe_allow_html=True)
        st.markdown("""
            <div class='sg-section-head'>
                <p class='sg-section-title'>🔍 Exploitability Matrix (Static Analysis Lite)</p>
            </div>
        """, unsafe_allow_html=True)

        with st.container(border=True):
            col_sa1, col_sa2 = st.columns([1, 2], gap="large")
            with col_sa1:
                target_func = st.text_input("Vulnerable System Function Name:", "execute_vulnerable_lookup", key="func_auto")
            with col_sa2:
                exploit_verdict = ml_intel.predict_exploitability_lite("app.py", target_func)
                st.markdown("<div style='height:26px;'></div>", unsafe_allow_html=True)
                if "HIGH" in exploit_verdict:
                    st.error(f"Exploitability Vector Result: {exploit_verdict}")
                else:
                    st.success(f"Exploitability Vector Result: {exploit_verdict}")
else:
    st.error("❌ Corporate inventory report not found. Please ensure your `pipeline.py` is executable and run it once to compile the data baseline.")
